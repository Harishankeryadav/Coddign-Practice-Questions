...
int visited[4] = {0}, visited_d1[20] = {0}, visited_d2[20] = {0};

bool isPossible(int row, int col)
{
    return visited[col] == 0 && visited_d1[row + col] == 0 && visited_d2[n + row - col] == 0;
}
...
void nQueen(int row)
{
    ...
    for (int i = 0; i < n; i++)
    {
        if (isPossible(row, i))
        {
            ...
            visited[i] = 1;
            visited_d1[row + i] = 1;
            visited_d2[n + row - i] = 1;

            nQueen(row + 1);
            s[row][i] = '.';
            visited[i] = 0;
            visited_d1[row + i] = 0;
            visited_d2[n + row - i] = 0;
        }
    }
}
